...
def detect_platform(raw_details):
    ...
    if isinstance(raw_details, dict):
        details_list = raw_details.get("details", [])
        if details_list:
            first = next((e for e in details_list if "_error" not in e), details_list[0])
...
def analyze_notes(details_path, self_details_path=None):
    ...
    # 兼容两种数据格式：
    #   新格式（dict）: { blogger: {...}, details: [...], _meta: {...} }
    #   旧格式（list）: [{ note: {...}, comments: {...} }, ...]
    if isinstance(raw_data, dict):
        raw_details = raw_data.get("details", [])
        blogger_info = raw_data.get("blogger", {})
    else:
