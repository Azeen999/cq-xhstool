...
def gen_distill_task(nickname, stats, top10, category_stats, tag_freq,
    ...
    # ---- mode 配置 ----
    if mode == "B":
        skill_dirname = f"{safe_name}_创作基因.skill"
        ...
        skill_dirname = f"{safe_name}_创作指南.skill"
    ...
    # 第二部分：任务一 — HTML 蒸馏报告
    # ================================================================
    # 输出路径：{博主名}_蒸馏报告/{博主名}_蒸馏报告.html
    report_folder = f"{safe_name}_蒸馏报告"
    report_filename = f"{safe_name}_蒸馏报告.html"
    report_output_path = f"{report_folder}/{report_filename}"
    
    lines += [
        f"",
        f"# 第二部分：任务一 — 生成 HTML 蒸馏报告",
        f"",
        f"**输出文件夹**：`{report_folder}/`",
        f"",
        f"**输出文件名**：`{report_output_path}`",
        f"",
        f"---",
        f"",
        f"## 技术要求",
...
def deep_analyze(analysis_path, nickname, output_dir, notes_details_path=None, mode="A"):
    ...
    if notes_details_path and os.path.exists(notes_details_path):
        ...
        if isinstance(raw_data, dict):
            raw_details = raw_data.get("details", [])
        else:
            raw_details = raw_data
        from analyze import detect_platform as _detect_platform
        platform = _detect_platform(raw_data)
        full_notes = []
        for item in raw_details:
            if "_error" in item:
                continue
            content = item.get("note") or item.get("video")
    ...
    # ---- 生成笔记拆解文件 ----
    all_notes = notes or top10
    for idx, note in enumerate(all_notes[:10], 1):  # 取前10条
        note_title = note.get("title", f"笔记{idx}")
        note_title_safe = safe_filename(note_title)
        note_folder_name = f"{idx:02d}_{note_title_safe}"
        note_folder = os.path.join(notes_dir, note_folder_name)
        ...
        if full_notes:
            nid = note.get("id", "")
            for fn in full_notes:
                if fn.get("noteId") == nid or fn.get("_feed_id") == nid:
                    content = fn.get("desc", content)
                    break
        content_path = os.path.join(note_folder, "正文.txt")
        ...
        images += note.get("imgs", []) or []
        images = list(dict.fromkeys(images))  # 去重
        
        images_path = os.path.join(note_folder, "图片列表.txt")
...
示例：
  ...
  python deep_analyze.py ./data/analysis.json "蔡不菜" -o ./output --mode B
    ...
    parser.add_argument("-o", "--output", default=r"D:\myproj\参考", help="输出目录")
